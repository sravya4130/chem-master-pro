...
import { relationsLessons } from '@/data/mathsData';
...
const LearnRelations = () => {
  ...
  const lesson = relationsLessons[currentLesson];
  ...
  if (mode === 'intro') {
    return (
      <AppLayout title="Relations & Functions">
        <div className="p-4 pb-8">
          <div className="bg-orange-500 rounded-2xl p-6 text-white mb-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h2 className="text-2xl font-bold">Relations & Functions</h2>
                <p className="opacity-80">Understand mappings and functions</p>
              </div>
            </div>
            {selectedTutor && <div className="bg-white/20 rounded-xl p-4"><p className="text-sm">"{selectedTutor.name} here! Relations and functions connect elements between sets. Let's master them!"</p></div>}
          </div>
          <div className="space-y-4">
            <Button onClick={() => setMode('learn')} className="w-full h-auto p-6 justify-start gap-4 bg-orange-500 hover:bg-orange-600">
              <BookOpen className="h-8 w-8" />
              <div className="text-left"><p className="font-bold text-lg">Learn First</p><p className="text-sm opacity-80">Study relations with examples</p></div>
            </Button>
            <Button onClick={() => navigate('/game/maths/relations-functions')} variant="outline" className="w-full h-auto p-6 justify-start gap-4 border-2">
  ...
  return (
    <AppLayout title={`Lesson ${currentLesson + 1}`}>
      <div className="p-4 pb-32">
        <div className="flex gap-1 mb-6">{relationsLessons.map((_, i) => <div key={i} className={`flex-1 h-2 rounded-full transition-colors ${i <= currentLesson ? 'bg-orange-500' : 'bg-muted'}`} />)}</div>
        {selectedTutor && (
          <div className="mb-6">
        ...
        <div className="bg-card rounded-2xl p-6 shadow-card mb-6">
          ...
          <div className="prose prose-sm text-foreground">{lesson.content.split('\n').map((line, i) => line.startsWith('**') && line.endsWith('**') ? <p key={i} className="font-bold mt-3">{line.replace(/\*\*/g, '')}</p> : line.startsWith('- ') ? <p key={i} className="ml-4">• {line.substring(2)}</p> : <p key={i} className="text-muted-foreground">{line}</p>)}</div>
        </div>
        <div className="bg-secondary rounded-2xl p-6">
          ...
          <p className="text-center"><span className="text-muted-foreground">Solution: </span><span className="font-bold text-orange-500">{lesson.example.solution}</span></p>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-lg border-t border-border">
        <div className="flex gap-3">
          ...
          <Button onClick={() => { stop(); currentLesson < relationsLessons.length - 1 ? setCurrentLesson(currentLesson + 1) : navigate('/game/maths/relations-functions'); }} className="flex-1">{currentLesson < relationsLessons.length - 1 ? 'Next' : 'Start Game'}<ArrowRight className="h-4 w-4 ml-2" /></Button>
        </div>
      </div>
    </AppLayout>
  );
