...
import { trigLessons } from '@/data/mathsData';
...
const LearnTrigonometry = () => {
  ...
  if (mode === 'intro') {
    return (
      <AppLayout title="Trigonometric Functions">
        <div className="p-4 pb-8">
          <div className="bg-cyan-500 rounded-2xl p-6 text-white mb-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="opacity-80">Master angles and ratios</p>
              </div>
            </div>
            {selectedTutor && <div className="bg-white/20 rounded-xl p-4"><p className="text-sm">"{selectedTutor.name} here! Trigonometry connects angles to ratios. Let's explore sin, cos, tan and more!"</p></div>}
          </div>
          <div className="space-y-4">
            <Button onClick={() => setMode('learn')} className="w-full h-auto p-6 justify-start gap-4 bg-cyan-500 hover:bg-cyan-600">
              <BookOpen className="h-8 w-8" />
              <div className="text-left"><p className="font-bold text-lg">Learn First</p><p className="text-sm opacity-80">Study trig with examples</p></div>
            </Button>
            <Button onClick={() => navigate('/game/maths/trigonometry')} variant="outline" className="w-full h-auto p-6 justify-start gap-4 border-2">
  ...
  return (
    <AppLayout title={`Lesson ${currentLesson + 1}`}>
      <div className="p-4 pb-32">
        <div className="flex gap-1 mb-6">{trigLessons.map((_, i) => <div key={i} className={`flex-1 h-2 rounded-full transition-colors ${i <= currentLesson ? 'bg-cyan-500' : 'bg-muted'}`} />)}</div>
        {selectedTutor && (
          <div className="mb-6">
        ...
        <div className="bg-secondary rounded-2xl p-6">
          ...
          <p className="text-center"><span className="text-muted-foreground">Solution: </span><span className="font-bold text-cyan-500">{lesson.example.solution}</span></p>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-lg border-t border-border">
        <div className="flex gap-3">
          ...
          <Button onClick={() => { stop(); currentLesson < trigLessons.length - 1 ? setCurrentLesson(currentLesson + 1) : navigate('/game/maths/trigonometry'); }} className="flex-1">{currentLesson < trigLessons.length - 1 ? 'Next' : 'Start Game'}<ArrowRight className="h-4 w-4 ml-2" /></Button>
        </div>
      </div>
    </AppLayout>
  );
...
export default LearnTrigonometry;
