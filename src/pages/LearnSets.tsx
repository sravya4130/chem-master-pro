...
import { setsLessons } from '@/data/mathsData';
...
const LearnSets = () => {
  ...
  const lesson = setsLessons[currentLesson];
    ...
    return (
      <AppLayout title="Sets">
        <div className="p-4 pb-8">
          <div className="bg-purple-500 rounded-2xl p-6 text-white mb-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="opacity-80">Master set theory fundamentals</p>
              </div>
            </div>
            {selectedTutor && <div className="bg-white/20 rounded-xl p-4"><p className="text-sm">"{selectedTutor.name} here! Sets are the foundation of mathematics. Let's learn about unions, intersections, and more!"</p></div>}
          </div>
          <div className="space-y-4">
            <Button onClick={() => setMode('learn')} className="w-full h-auto p-6 justify-start gap-4 bg-purple-500 hover:bg-purple-600">
              <BookOpen className="h-8 w-8" />
              <div className="text-left"><p className="font-bold text-lg">Learn First</p><p className="text-sm opacity-80">Study set theory with examples</p></div>
            </Button>
            <Button onClick={() => navigate('/game/maths/sets')} variant="outline" className="w-full h-auto p-6 justify-start gap-4 border-2">
  ...
  return (
    <AppLayout title={`Lesson ${currentLesson + 1}`}>
      <div className="p-4 pb-32">
        <div className="flex gap-1 mb-6">{setsLessons.map((_, i) => <div key={i} className={`flex-1 h-2 rounded-full transition-colors ${i <= currentLesson ? 'bg-purple-500' : 'bg-muted'}`} />)}</div>
        {selectedTutor && (
          <div className="mb-6">
        ...
        <div className="bg-secondary rounded-2xl p-6">
          ...
          <p className="text-center"><span className="text-muted-foreground">Solution: </span><span className="font-bold text-purple-500">{lesson.example.solution}</span></p>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-lg border-t border-border">
        <div className="flex gap-3">
          ...
          <Button onClick={() => { stop(); currentLesson < setsLessons.length - 1 ? setCurrentLesson(currentLesson + 1) : navigate('/game/maths/sets'); }} className="flex-1">{currentLesson < setsLessons.length - 1 ? 'Next' : 'Start Game'}<ArrowRight className="h-4 w-4 ml-2" /></Button>
        </div>
      </div>
    </AppLayout>
  );
