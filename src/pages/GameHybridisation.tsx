...
import { hybridisationQuestions, HybridisationQuestion } from '@/data/hybridisationData';
import { ArrowRight, Lightbulb, X, Check, Trophy, RotateCcw, Atom } from 'lucide-react';
import { toast } from 'sonner';
import { cn } from '@/lib/utils';

const GameHybridisation = () => {
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState<HybridisationQuestion[]>([]);

  useEffect(() => {
    const shuffled = [...hybridisationQuestions].sort(() => Math.random() - 0.5);
  ...
  const nextQuestion = () => {
    if (currentQuestionIndex < shuffledQuestions.length - 1) {
      ...
      setUserProgress(prev => ({
        ...
        completedTopics: prev.completedTopics.includes('hybridisation') 
          ? prev.completedTopics 
          : [...prev.completedTopics, 'hybridisation']
      }));
    }
  };
  ...
  if (gameState === 'ready') {
    return (
      <AppLayout title="Hybridisation Quiz">
        <div className="p-4 pb-8 flex flex-col min-h-[calc(100vh-56px)]">
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            <div className="w-20 h-20 rounded-full bg-topic-blue/20 flex items-center justify-center mb-6 animate-bounce-in">
              <Atom className="h-10 w-10 text-topic-blue" />
            </div>
            <h2 className="text-2xl font-bold mb-2">Hybridisation Quiz</h2>
            <p className="text-muted-foreground mb-6">
              Test your knowledge of orbitals and quantum numbers!
            </p>
                ...
                {[20, 30, 45, 60, 90].map((sec) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl font-medium transition-all ${
                      timerDuration === sec
                        ? 'bg-topic-blue text-primary-foreground'
              ...
              <div className="bg-card rounded-xl p-4 shadow-card mb-6 max-w-sm">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm text-left">
                    "{selectedTutor.name} says: Remember, the number of hybrid orbitals equals the number of sigma bonds + lone pairs!"
                  </p>
                </div>
              </div>
            )}
            ...
            <Button
              ...
              className="h-14 px-8 text-lg font-bold gap-2 bg-topic-blue hover:bg-topic-blue/90"
            >
              Start Quiz
        ...
        <div className="p-4 pb-8 flex flex-col min-h-[calc(100vh-56px)]">
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <Button onClick={() => navigate('/topics')} className="bg-topic-blue hover:bg-topic-blue/90">
                Back to Topics
              </Button>
            </div>
          </div>
            ...
            {shuffledQuestions.map((_, i) => (
              <div
                ...
                className={`flex-1 h-2 rounded-full transition-colors ${
                  ...
                  i === currentQuestionIndex ? 'bg-topic-blue' : 'bg-muted'
                }`}
              />
            ))}
          </div>
        ...
        {currentQuestion && (
          <div className="bg-card rounded-2xl p-6 shadow-card">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className={`text-xs px-2 py-1 rounded-full bg-topic-blue/20 text-topic-blue`}>
            ...
            {currentQuestion.molecule && (
              <div className="bg-secondary rounded-xl p-4 mb-4">
                <p className="font-mono text-xl text-center text-topic-blue">{currentQuestion.molecule}</p>
              </div>
            )}
                  ...
                  {currentQuestion.options.map((option, i) => (
                    <button
                      ...
                      className={cn(
                        ...
                        "border-2 border-transparent hover:border-topic-blue/50"
                      )}
                    >
                      {option}
                    </button>
            ...
            {gameState === 'checking' && (
              <div className={`rounded-xl p-6 text-center ${
                ...
                <Button onClick={nextQuestion} className="bg-topic-blue hover:bg-topic-blue/90">
...
export default GameHybridisation;
