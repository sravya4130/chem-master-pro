...
import { setsQuestions, relationsQuestions, trigQuestions, MathQuestion } from '@/data/mathsData';
...
const GameMaths = () => {
  ...
  const getQuestions = (): MathQuestion[] => {
    switch (topicId) {
      case 'sets': return setsQuestions;
      case 'relations-functions': return relationsQuestions;
      case 'trigonometry': return trigQuestions;
      default: return setsQuestions;
    }
  };
  ...
  const getTopicName = () => {
    switch (topicId) {
      ...
      case 'relations-functions': return 'Relations & Functions';
  ...
  const getColor = () => {
    switch (topicId) {
      ...
      case 'relations-functions': return 'orange';
      case 'trigonometry': return 'cyan';
            ...
            <div className="mb-8">
              ...
              <div className="flex gap-2 flex-wrap justify-center">
                {[30, 60, 120, 180, 300].map((sec) => (
                  <button key={sec} onClick={() => setTimerDuration(sec)} className={`px-4 py-2 rounded-xl font-medium transition-all ${timerDuration === sec ? `bg-${getColor()}-500 text-white` : 'bg-secondary hover:bg-secondary/80'}`}>
        ...
        <div className="p-4 pb-8 flex flex-col min-h-[calc(100vh-56px)]">
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <Button onClick={() => navigate('/topics/maths')}>Back to Topics</Button>
            </div>
          </div>
        </div>
      </AppLayout>
